...
def train(args, **kwargs):
    ...
    if not torch.cuda.is_available() or args.cpu:
        ...
        checkpoint_pretrained = torch.load(args.pretrained_model_path, map_location=lambda storage, location: storage)
    else:
        device = torch.device('cuda:0')
        checkpoint_pretrained = torch.load(args.pretrained_model_path)

    network_pretrained = get_model(args.arch).to(device)
    network_pretrained.load_state_dict(checkpoint_pretrained.get('model_state_dict'))
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            for batch_id, (feat, targ, _, _) in enumerate(train_loader):
                ...
                pretrained_pred = network_pretrained(feat)
                ...
                loss_ss = criterion_cosineEmbedded(feat_contrast_pred, pred_contrast, torch.ones(1, device=device))
