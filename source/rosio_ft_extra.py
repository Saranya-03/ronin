...
def train(args, **kwargs):
    ...
    total_params = network_p.get_num_params()
    ...
    if args.continue_from is not None and osp.exists(args.continue_from):
        ...
        network_p.load_state_dict(checkpoints.get('model_state_dict'))
    ...
    if val_loader is not None:
        init_val_targ, init_val_pred = run_test(network_p, val_loader, device)
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            for batch_id, (feat, targ, _, _) in enumerate(train_loader):
                ...
                feat_contrast_copy = feat_contrast.detach().requires_grad_(False)

                train_outs.append(pred.cpu().detach().numpy())
                train_targets.append(targ.cpu().detach().numpy())

                targ_contrast = targetTransformationModule(targ,random_degrees,device)
                targ_contrast_copy = targ_contrast.detach().requires_grad_(False)
            ...
            if val_loader is not None:
                network_p.eval()
                val_outs, val_targets = run_test(network_p, val_loader, device)
