    ...
    try:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            for batch_id, (feat, targ, _, _) in enumerate(train_loader):
                ...
                feat_copy = feat.clone().detach().requires_grad_(False)
                # feat_for_pretrain = feat.clone().detach().requires_grad_(False)
                pred = network(feat)
                pred_copy = pred.clone().detach().requires_grad_(False)
                psudo = network_p(feat)
                psudo_copy = psudo.clone().detach().requires_grad_(False)
                feat_contrast, random_degrees = featTransformationModule(feat_copy, device)
                feat_contrast_copy = feat_contrast.clone().detach().requires_grad_(False)

                train_outs.append(pred.cpu().detach().numpy())
                train_targets.append(targ.cpu().detach().numpy())
                train_psudo.append(psudo.cpu().detach().numpy())

                pred_contrast = targetTransformationModule(pred_copy, random_degrees, device)
                pred_contrast_copy = pred_contrast.detach().requires_grad_(False)
                psudo_contrast = targetTransformationModule(psudo_copy,random_degrees,device)
                psudo_contrast_copy = psudo_contrast.clone().detach().requires_grad_(False)
                ...
                # for i in range(len(psudo_c)):
                #     if (i==0):
                #         loss_t=-criterion_cosine(torch.unsqueeze(v_2[i], 0), torch.unsqueeze(psudo_c[i], 0)).requires_grad_(True)
                #     else:
                #         if (torch.norm(psudo_c[i]) > 0.5):
                #             loss_t -=criterion_cosine(torch.unsqueeze(v_2[i], 0), torch.unsqueeze(psudo_c[i], 0)).requires_grad_(True)
                #         else:
                #             loss_t += 0
                ...
                loss_t=criterion(v_2,pred_contrast_copy)
                loss_t=torch.mean(loss_t)

                # loss_t = loss_t / len(pred)
                train_rot_mse.append(loss_t.cpu().detach().numpy())

                total_loss = args.n1 * loss_p + args.n2 * loss_t
